...
class Microphone:
    ...
    def change_sample_rate(self, input_path, output_path: str, target_rate: int = 16000):
        """
        Changes the sample rate of an audio file and saves it to a new file.

        Args:
            input_path (str): The path to the input audio file.
            output_path (str): The path to save the resampled audio file.
        ...
        try: 
            ...
            if original_sample_rate == target_rate:
                print(f'Audio already has the target sample rate of {target_rate}Hz')
                sf.write(output_path, audio_data, original_sample_rate)
                return

            # Calculate the number of samples for the target rate
            num_samples = int(len(audio_data)) * target_rate / original_sample_rate

            # Resample the audio data
            resampled_audio = resample(audio_data, int(num_samples))
            sf.write(output_path, resampled_audio, target_rate)
            print(f'✅ Sample rate changed from {original_sample_rate}Hz to {target_rate}Hz and saved to {output_path}')
        except Exception as e:
